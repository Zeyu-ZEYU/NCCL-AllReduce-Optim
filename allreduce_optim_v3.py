    ...
    while True:
        for train_input, train_label in data_loader:
            ...
            for i in range(4):
                pg = pgroups[i]
                if i != pgroup_index:
                    src_rank = rank - rank % 4 + i
                    for param in pg:
                        ah = torch.distributed.broadcast(param.grad, src=src_rank, group=local_group, async_op=True)
                        async_handlers.append(ah)
                else:
                    for param in pg:
                        ah = torch.distributed.broadcast(param.grad, src=rank, group=local_group, async_op=True)
                        async_handlers.append(ah)
            for ah in async_handlers:
                ah.wait()
